...
> - **Output home:** `docs/brainstorms/<topic>-requirements.md` (the dir exists).
> - **Stack:** this is a TypeScript service (Postgres + pgvector, MCP/HTTP). Use
>   repo-relative `*.ts` paths in examples — never Rails/`.rb` or Figma/UI
>   references; there is no front-end.
> - **Hand-off menu (Phase 4 / `references/handoff.md`):** keep **Plan with
>   `/ce-plan`** and **Build with `/ce-work`**. **Drop** the un-vendored options —
>   `ce-doc-review`, Proof/`ce-proof`, and the `ce-slack-researcher` Slack-context
>   step. If one is offered, omit it silently.
