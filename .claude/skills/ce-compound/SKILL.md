...
> **Local config (jesusfilm-rag) — read first.** This is a vendored, retuned copy
> (see `compound-engineering.local.md`). Several hand-offs the upstream skill
> references are **not** available here — wherever the steps below mention them,
> **skip that step** and continue:
> - **`ce-sessions`** (session-history search) — not vendored. Ignore the
>   "Pre-resolved context" block, the Full-Mode session-history question, and
>   Phase 1 step 4. Run as if the user always declined session history.
> - **`ce-compound-refresh`** (Phase 2.5) — not vendored. Never invoke it; do not
>   surface a "Refresh recommendation". Skip Phase 2.5 entirely.
> - **Phase 3 specialized reviewers** and the "Applicable Specialized Agents"
>   fleet (`ce-kieran-*`, `ce-performance-oracle`, etc.) — not vendored. Skip
>   Phase 3 entirely in every mode.
> - **Validation** — use `pnpm check:solutions` (repo-native), **not** the
>   `scripts/validate-frontmatter.py` referenced in Phase 2.
> - **Discoverability target** — this repo's instruction file is `AGENT.md`
>   (there is no `CLAUDE.md`); `AGENT.md` already surfaces `docs/solutions/`, so
>   the Discoverability Check normally passes with no edit.
> - **After writing the doc**, add its one-line row to `docs/solutions/README.md`
>   between the `INDEX:START`/`INDEX:END` markers, then run
>   `pnpm check:solutions` — green is a precondition (the CI gate enforces it).
...
8. **Add the index row.** In `docs/solutions/README.md`, insert one line for the new doc between the `<!-- INDEX:START -->` and `<!-- INDEX:END -->` markers (remove the "No learnings captured yet." placeholder line if it is still there). Format: `- [title](category/filename.md) — one-line summary · \`component\` · YYYY-MM-DD`. (Skip when updating an existing doc that is already indexed.)
9. **Run `pnpm check:solutions`** — the repo-native gate (replaces the upstream `scripts/validate-frontmatter.py`). It catches the silent-corruption parser-safety traps the prose rules miss (malformed `---` delimiters, unquoted ` #` and `: ` in scalar values), verifies the four anchor frontmatter fields (`title`, `date`, `problem_type`, `component`) are present, and confirms the doc is linked in the Lessons Index. Exit 0 = parser-safe and indexed; exit 1 = stderr names each problem. Fix (quote the value, add the missing field, or add the index row) and re-run until exit 0. Do not declare success while it fails — CI runs the same check and will block the merge.
