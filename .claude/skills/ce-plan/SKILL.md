...
> - **Output home:** `docs/plans/<slug>.md`. Upstream requirements live in
>   `docs/brainstorms/`. Both dirs exist.
> - **Issue tracker:** `project_tracker: github` (`JesusFilm/jesusfilm-rag`). Use
>   `gh issue create`; ignore the Linear branch.
> - **Honor this repo's bars** in the plan: the three-context import law +
>   DI/composition-root (architecture.md §5), fakes-only unit tests, the
>   file-size cap, read-only-at-MCP, and `mechanism-not-policy`. A plan that locks
>   a hard-to-reverse direction must also produce an **ADR** in `docs/decisions/`.
> - **Executor hand-off:** route per-source pipeline work (acquire → ingest →
>   retrieve → eval for one source) to **`/slice`**; route everything else to
>   **`/ce-work`**.
> - **Not vendored — skip these:** the Phase 1.1 parallel **research-agent**
>   dispatch, the Phase 5.3 **deepening** agent-research pass, the mandatory
>   **`ce-doc-review`** headless review and its menu option, and **`ce-proof`**.
>   Proceed from local context + user input straight to writing the plan; mark
>   plans `status: active`.
> - **Stack:** TypeScript; use repo-relative `*.ts` paths, never `.rb`.
