...
> **Local config (jesusfilm-rag) — read first.** Vendored, retuned copy (see
> `compound-engineering.local.md`). Adjustments to the upstream flow:
> - **Scope — not the per-source pipeline.** `/ce-work` is the **generic**
>   executor (infra, tooling, refactors, eval-harness, process/docs). The
>   canonical per-source vertical slice (acquire → ingest → retrieve → eval for
>   one source) has its own executor with checkpoint state: **`/slice`**. If the
>   work is a source slice, stop and use `/slice` instead.
> - **Verify gate (hard, before opening a PR):**
>   `pnpm depcruise && pnpm lint && pnpm typecheck && pnpm test`, then
>   `pnpm check:solutions`, and `pnpm eval` when acquisition/ingestion/retrieval
>   or the corpus changed. This replaces any upstream `/simplify`/lint-agent step.
> - **Branch/worktree:** use the repo convention
>   `git worktree add .claude/worktrees/<name> origin/main` (branch off
>   `origin/main`, never work on `main`). There is no `ce-worktree` skill.
> - **Commit / push / PR:** plain `git` with **Conventional Commits**
>   (`feat(scope): …`) — enforced by the commitlint `commit-msg` hook and the
>   `pr-title` CI check. There is no `ce-commit-push-pr`/`ce-commit`; do those
>   steps directly. PR titles must be Conventional Commits (squash-merge subject).
> - **Review:** harness-native review + a fresh cold-subagent diff read against
>   this repo's bars. The `ce-code-review` fleet and `ce-demo-reel` evidence
>   capture are **not vendored** — skip them (this is a headless service, no UI to
>   screenshot).
> - **Issue tracker:** `project_tracker: github` (`JesusFilm/jesusfilm-rag`).
> - **After merge:** run **`/ce-compound`** to capture any learning into
>   `docs/solutions/`, and report the ship with both flags (Reviewed ✅ +
>   Compounded ✅) per `docs/workflow/ways-of-working.md`.
