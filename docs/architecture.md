...
| 12 | Walled-source fetch strategy *(added 2026-07-22)* | **Firecrawl as an optional per-source fetch strategy** — a walled source (content behind a JS bot wall plain HTTP can't pass) declares `fetchStrategy: "firecrawl"` in its registry crawl policy; absent = plain HTTP, so Firecrawl is strictly opt-in and the nine existing sources are untouched. Strictly a transport behind the `Fetcher` port (raw rendered HTML, Firecrawl-side cache disabled); discovery/extraction/chunking stay in-repo. Static slice-time choice for ALL a source's requests — no runtime fallback, no per-request mixing; registry-only, never persisted (no migration). `/slice` stage-1 classifies a wall by the Cloudflare block-page signature only (never the `challenge-platform` script ref — served CF pages carry it too). Implements FOLLOW-UP G ([#8](https://github.com/JesusFilm/jesusfilm-rag/issues/8)) via [#104](https://github.com/JesusFilm/jesusfilm-rag/issues/104). | [ADR-0012](./decisions/0012-firecrawl-fetch-strategy-walled-sources.md) |
| 13 | Walled-source prod promotion *(added 2026-07-28)* | **Bulk-copy `raw_documents` local→prod — an optional path for walled/metered sources.** For a Firecrawl-walled source, `acquire:production` would pay the metered acquire a second time; instead acquire once locally, validate, then `scripts/copy-raws.sh` copies the flat FK-free `raw_documents` (omitting `id`/`ingested_at` so prod regenerates the uuid and leaves rows pending) and `index:production` embeds in prod. **Embeds twice, acquires once**; its justifying case is walled/metered sources (a free source keeps `acquire:production` — bulk-copy works there but saves nothing); runs within the existing local-or-VM promotion model, no new risk class. Spec [#115](https://github.com/JesusFilm/jesusfilm-rag/issues/115), built [#120](https://github.com/JesusFilm/jesusfilm-rag/pull/120), validated on everystudent en/ar/fr. | [ADR-0014](./decisions/0014-bulk-copy-raw-documents-to-prod.md) |
| 14 | Ingestion throughput *(added 2026-08-07)* | **Bounded concurrency across distinct document identities plus bulk per-document writes.** Sources are established before workers start; duplicate `(sourceKey, canonicalUrl)` rows retain input order; first failure stops new scheduling and active work drains; each successful raw row is marked independently. `replaceDocument` remains one transaction while chunks and embeddings use two multi-row inserts. | [ADR-0017](./decisions/0017-bounded-concurrent-ingestion.md) |
...
- **FOLLOW-UP G — bot-walled / JS-rendered sources (Cloudflare / Angular / Framer).** → [#8](https://github.com/JesusFilm/jesusfilm-rag/issues/8). _**Update 2026-07-22 — capability shipped:** per-source `fetchStrategy: "firecrawl"` behind the `Fetcher` port ([ADR-0012](decisions/0012-firecrawl-fetch-strategy-walled-sources.md), [#104](https://github.com/JesusFilm/jesusfilm-rag/issues/104); see changelog row 12). The blocked everystudent slice resumes as its own piece of work._
...
- **FOLLOW-UP O — the eval scripts inherit the FAST-FAIL query embed retry policy; they need a batch posture.** _`docs/ops/embed-retry-policy.md` deliberately runs two postures: patient for corpus embedding (`EMBED_MAX_ATTEMPTS` 10, because one blip aborting `pnpm index` throws away hours — #64) and fast-fail for query embedding (`QUERY_EMBED_MAX_ATTEMPTS` 2, `QUERY_EMBED_TIMEOUT_MS` 4 s, because a caller behind `/v1/search` has already given up). The doc files **`pnpm eval` / `pnpm eval:production` under the QUERY column** — correct by wiring (they drive the same `Retriever`), wrong by workload: a 118-case offline eval has no latency SLA and **no resume**, so a single transient OpenRouter timeout discards the whole run. It killed two full runs in slice #9 (2026-07-25), each after ~100 completed cases, and was worked around with `QUERY_EMBED_MAX_ATTEMPTS=8 QUERY_EMBED_TIMEOUT_MS=25000 pnpm eval` — env only, no code change, serving path untouched. This is the #64 lesson (patience where a long run is at stake) never having been applied to the eval path. Candidate fixes, cheapest first: (a) have `scripts/eval.ts` default the query embedder to a batch posture, overridable by env; (b) a third wired instance (`batchQueryEmbedder`) so the posture is explicit at the wiring seam rather than per-script; (c) resume/checkpointing for eval runs, which also fixes re-running a partially-failed eval. **Not a serving-path change** — `/v1/search` must keep failing fast._
