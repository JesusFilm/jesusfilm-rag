...
| 10 | Language labeling *(added 2026-07-09; thresholds + `null` policy 2026-07-13)* | **Per-document, content-based language detection at ingest** — `documents.language` is detected from the cleaned content by an in-process `tinyld` detector (invariant 6), never sourced from `source.languages`/URL/`<html lang>`. `source.languages` becomes the declared/expected set. Sources are split by **domain**; language is a per-document property. Below the **500-char detection floor** or **0.75 confidence gate** the label is stored **`null`** (never guessed, never defaulted — supersedes ADR-0006's low-confidence fallback); `null` rows form [#73](https://github.com/JesusFilm/jesusfilm-rag/issues/73)'s cleanup worklist. Fixes the FamilyLife `es`→`en` mislabel with a label-only backfill (no re-embed). Tracked by [#68](https://github.com/JesusFilm/jesusfilm-rag/issues/68). The [#73](https://github.com/JesusFilm/jesusfilm-rag/issues/73) corrective sweep then labels in-set below-floor docs and an **established label is never nulled out on re-ingest** (`replaceDocument` writes `coalesce(new, existing)`; ADR-0008). | [ADR-0006](./decisions/0006-per-document-language-detection.md) · [ADR-0007](./decisions/0007-language-decision-thresholds-null-policy.md) · [ADR-0008](./decisions/0008-language-label-lifecycle.md) |
