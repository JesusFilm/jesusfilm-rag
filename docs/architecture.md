...
- **FOLLOW-UP J — filtered vector search under-recalls in a large corpus (HNSW `ef_search` post-filter drops in-scope docs below out-of-scope neighbors); raise ef_search / iterative scan / pre-filter for selective scopes.** → [#17](https://github.com/JesusFilm/jesusfilm-rag/issues/17) _**Sharpened (2026-07-09, slice #7):** at ~24.6 k chunks this manifests as **flakiness, not a deterministic failure**. `tests/retrieval.integration.test.ts` failed twice (its `match` fixture, cosine 1.0, lost rank 1 to the `orthogonal` fixture at cosine 0.3) and then passed on the next two runs with **no code and no DB change** — the fixtures are inserted/deleted per run, so each run rebuilds a slightly different HNSW neighbourhood. A green CI run therefore proves nothing about #17 (CI's DB is near-empty); only a populated DB exercises it. Treat that test as a canary, not a gate._
...
- **FOLLOW-UP L — source discovery endpoint (`GET /v1/sources`) so external consumers can configure `allowedSourceKeys` / `excludedSourceKeys` / `perSourceCaps` without out-of-band knowledge.** → issue TBD. _Shape: `[{ key, name, docCount, lastIndexedAt, scopeDescription }]`. Scope-aware — a bearer token only sees the sources it's authorized for (intersection with `allowedSourceKeys`, same security model as `POST /v1/search`). Read-only over the existing `sources` table via a thin `listSources()` addition to `CorpusSearchStore` (no new boundary crossings). **Effective prerequisite for FOLLOW-UP I's `perSourceCaps` and FOLLOW-UP E's `excludedSourceKeys` being externally usable** — internal consumers (us) can hardcode keys today, third-party consumers (Mastra agents, NextSteps, chatbots) cannot. Worth shipping alongside or just before whichever of E/I lands first._
- **FOLLOW-UP M — per-document language detection at ingest (`documents.language` is currently stamped from `SourceEntry.languages[0]`, so a source may hold exactly ONE language).** → issue TBD. _Discovered in slice #7 (cru). `normalize()` (`src/ingestion/normalize.ts`) sets `const language = entry.languages[0] ?? "en"` for every document, so the only correct multilingual model today is **one source per language** (`thelife-fr`, `thelife-zh`, `cru-es`). Two consequences we are living with: (1) cru's `/us/en/train-and-grow/language-resources/` — genuine gospel content in ~28 languages — is **excluded from the `cru` crawl**, because folding it into an `languages:["en"]` source would silently mis-tag every page `en` and poison the language-scoped eval; (2) `familylife` **declares** `languages:["en","es"]` but its Spanish (`/us-latinos/`) has never been ingested, and if it were, it would be tagged `en`. So the "one source, many languages" pattern is aspirational, not implemented. **Fix:** detect language per document at normalize time and set `documents.language` from that, demoting `SourceEntry.languages` to a declared/expected set rather than the stamp. **Detect from the extracted body** — it is already persisted (`raw_documents.raw_content`, `documents.content`), so detection needs **no schema change, no re-crawl, and no re-embed**; backfill is a `documents.language` column update. **`<html lang>` is NOT a usable signal** (measured 2026-07-09): `cru.org/mx/es/.../10-pasos-basicos/intro-the-uniqueness-of-jesus.html` declares `lang="es-mx"` while serving an untranslated **English** body, exactly like the URL path does. Both `<html lang>` and a `by-path` rule are therefore **priors, not authorities** — body detection must be able to override them, and a prior/detection **disagreement should be logged**, since that check alone would have caught the `/10-pasos/` pages. Fixture: `/us/en/.../4-prayer.html` (en-us→EN), `/mx/es/conoce-a-dios/jesus-dios-o-simplemente-buen-hombre.html` (es-mx→ES), `/mx/es/.../10-pasos-.../intro-the-uniqueness-of-jesus.html` (es-mx→**EN**). **Companion rule:** one domain = one source, so a source may legitimately hold several languages and per-doc detection becomes mandatory rather than optional. **A `by-path` rule is provably insufficient:** a body-language audit of all 537 staged `cru.org/mx/es/` documents (with `/10-pasos/` already blocked) found **489 Spanish, 3 mixed, 4 thin — and 39–41 English**, clustered in `crecer-y-equipar/comparte-evangelio` (31 of 98) and `.../vida-y-relaciones` (10 of 194). They share zero `body_hash` with any `us/en` document, i.e. they are untranslated originals that must be kept and labelled `en`. `by-path` would mislabel all 39. A 30-page spot-check of the same path had reported "0 English" — **sampling cannot establish a per-document property**, which is exactly why this must be a per-document check with prior/detection disagreements logged._
