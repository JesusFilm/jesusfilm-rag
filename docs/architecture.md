...
- **FOLLOW-UP F — discovery-crawl policy (resolve "generic crawler vs per-source").** STATUS deferred this until 2–3 sources reveal the pattern; jfa's registry (examined 2026-05-25, see [docs/jfa-registry-findings.md](./jfa-registry-findings.md)) **reveals it.** The proven shape is a declarative policy — `seeds` (often the `sitemap.xml`) + `allow[]`/`block[]`/`articleHints[]` regexes + `contentSelectors[]` + `sitemaps[]` — with the crawler following links from the seeds, keeping URLs that match `allow ∧ articleHints` and dropping `block`. Our current Acquisition only fetches the exact `seedPaths` listed, which is fine for small static sources but a dead end for the large corpora (GotQuestions ~1500, Sightline ~2500, FamilyLife ~15000). **Adopt when the first large source is taken**; until then small curated scopes (e.g. `cru-10-basic-steps`, 12 URLs) fit the current `seedPaths` code. Change surface: extend `CrawlPolicy` (registry) + add a discovery step to the Acquisition context. Pairs with the deferred robots.txt + http-cache (§3/§8).
- **FOLLOW-UP G — Cloudflare / bot-walled & JS-rendered sources.** Some sources block plain HTTP fetching. Confirmed 2026-05-25: **EveryStudent** serves a Cloudflare JS `challenge-platform` on every content page (homepage 200, content 403) — and jfa's own notes already say it *"actively blocks bot traffic… ingestion is via curated article lists, approved archives, or official exports"* (record: [docs/slices/everystudent.md](./slices/everystudent.md)). Separately, **KnowGod** (Angular) and **Victory Beyond the Cup** (Framer) are JS-rendered shells a plain fetch can't read. Bypass options, by cost: (a) curated archives / official exports; (b) a headless-browser (Playwright) `Fetcher` adapter behind the existing port — heavy dep, may still lose to a managed challenge; (c) authorized owner feed/API. **Decide when a walled source becomes *required* coverage**; today only EveryStudent (1 of the short list) is walled, so it stays `blocked`. The `Fetcher` port already isolates this — a browser fetcher slots in without touching the Acquisition context (§5).
