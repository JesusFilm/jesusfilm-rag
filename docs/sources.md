...
| FamilyLife | https://www.familylife.com | HTML | Acquiring | — | **Slice #6** (`slice/familylife`), started 2026-06-03. WordPress VIP behind `sitemaps.xml` (30 child sitemaps); reuses slice #3/#4/#5 discovery crawler (FOLLOW-UP F). Recon 2026-06-03: open robots, no challenge wall, sample article 300 KB, selectors `.the-content` / `.single-content`. **Recommended scope: posts only** — `post-sitemap1` (939) + `post-sitemap2` (997) + `post-sitemap3` (394) = ~2,330 articles under `/articles/topics/...`. Sub-brand sitemaps (Art of Marriage, Blended, Stepping Up, etc.) deferred — Cru-style sub-keys later if needed. See `docs/slices/familylife.md`. |
