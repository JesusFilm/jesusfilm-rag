...
| Sightline Ministry | https://sightlineministry.org | HTML | Acquiring | — | **Slice #4** (`slice/sightline-ministry`), started 2026-05-27. Same WP/Yoast shape as jesusfilm.org → **reuses slice #3's discovery crawler unchanged** (no new code). Probed 2026-05-27: serves 200, empty `Disallow:`, no challenge wall. `sitemap_index.xml` → 16 child sitemaps; teaching content = **post (415)** + **daily-devo (1001)**, both on `.o-longform-content__content`; resources (45) use a `.o-principle-block` card/hub template (excluded); asset (470) + taxonomy (post_tag 124 / category 28 / author / event / job) are non-article. Scoped by seeding specific content sitemaps (posts live at bare-root `/<slug>/`, so the index would pull in the noise). **Paused at Stage-1 budget gate** — operator picks posts-only (~414) vs posts+devotionals (~1,414). See `docs/slices/sightline-ministry.md`. |
