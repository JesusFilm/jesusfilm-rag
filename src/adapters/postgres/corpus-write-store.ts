    ...
    await this.db.transaction(async (tx) => {
      ...
      const [documentRow] = await tx
        ...
        .onConflictDoUpdate({
          ...
          set: {
            ...docMutable,
            // On re-ingest NEVER null out an established language: a fresh
            // decision of `null` (e.g. a re-embed of a below-floor doc, where
            // decideLanguage abstains) must not clobber a label a human or the
            // #73 sweep already set. A confident NEW detection still wins; only
            // null is prevented from overwriting. Mirrors the sweep's never-blank
            // policy (resolve-language.ts), so a re-embed preserves swept labels.
