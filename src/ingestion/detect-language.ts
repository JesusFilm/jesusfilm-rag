 ...
 *
 * This module is the pure detection primitive only. The confidence gate +
 * declared-set cross-check (what to do when confidence is low, or the detected
 * language falls outside `source.languages`) lives in `normalize.ts`, which owns
 * the `documents.language` decision. If an LLM escalation is ever added for
